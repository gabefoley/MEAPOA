...
class PairHMM():
    def __init__(self, profiles, tau, epsilon, delta, subsmat, log_transform):
        self.tau = tau
    ...
    def initialise_probs(self):
        ...
        self.emissionX = self.delta
        self.emissionY = self.delta
    ...
    def fillVM(self, i, j):
        ...
        if self.log_transform:
            ...
            curr_transitionMM = float('-Inf') if self.vM[i - 1][j - 1] == -1 else np.log(self.transitionMM) + \
                                                                                  self.vM[i - 1][j - 1]
            curr_transitionXM = float('-Inf') if self.vX[i - 1][j - 1] == -1 else np.log(self.transitionXM) + \
                                                                                  self.vX[i - 1][j - 1]
            curr_transitionYM = float('-Inf') if self.vY[i - 1][j - 1] == -1 else np.log(self.transitionYM) + \
                                                                                  self.vY[i - 1][j - 1]
            ...
            curr_transitionMM = float('-Inf') if self.vM[i - 1][j - 1] == -1 else self.transitionMM * self.vM[i - 1][
                j - 1]
            curr_transitionXM = float('-Inf') if self.vX[i - 1][j - 1] == -1 else self.transitionXM * self.vX[i - 1][
                j - 1]
            curr_transitionYM = float('-Inf') if self.vY[i - 1][j - 1] == -1 else self.transitionYM * self.vY[i - 1][
                j - 1]
    ...
    def fillVX(self, i, j):
        ...
        if self.log_transform:
            curr_transitionXX = float('-Inf') if self.vX[i][j - 1] == -1 else np.log(self.transitionXX) + self.vX[i][
                j - 1]
            curr_transitionMX = float('-Inf') if self.vM[i][j - 1] == -1 else np.log(self.transitionMX) + self.vM[i][
                j - 1]
    ...
    def fillVY(self, i, j):
        ...
        if self.log_transform:
            ...
            curr_transitionYY = float('-Inf') if self.vY[i - 1][j] == -1 else np.log(self.transitionYY) + \
                                                                              self.vY[i - 1][j]
            curr_transitionMY = float('-Inf') if self.vM[i - 1][j] == -1 else np.log(self.transitionMY) + \
                                                                              self.vM[i - 1][j]
